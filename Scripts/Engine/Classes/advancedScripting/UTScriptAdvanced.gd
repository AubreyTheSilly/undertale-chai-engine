		...
		while t < script[l].size():
			...
			if token is Lexer.FunctionToken:
				...
				if _get_variable(str(token.value)):
					...
					if next_token.type == Lexer.TokenType.EQUALS and t < script[l].xeesize()-2:
...
func _get_variable(variable:String):
...
func _set_variable(variable:String,value:Variant):
	if variables.has(node.name):
		if variables[node.name].has(variable):
			return variables[node.name][variable]
	if global_variables.has(variable):
		global_variables[variable].value = value
	else:
		push_error("Variable \""+variable+"\" does not exist")

func _convert_variables(parameters : Array,exceptions : Array = []) -> Array:
	var param = parameters
	var index = 0
	
	while index < param.size():
		if _get_variable(str(param[index].value)) and !exceptions.has(index):
		...
		elif param[index] is Lexer.FunctionToken:
	...
	match token.value:
		"print":
			if token.params.size() == 0:
				...
				token.params = await _convert_variables(token.params)
				var output := ""
		...
		"initvar":
			if token.params.size() != 2 and token.params.size() != 3:
				for i in token.params.size():
					print(i.value)
				push_error('Line '+str(total_l)+': initvar() requires between two and three parameters')
				return
			token.params = await _convert_variables(token.params,[0,1])
			...
			if variables.has(token.params[0].value):
				push_warning('Line '+str(total_l)+': Variable '+token.params[0].value+' is already defined and will be overwritten')
			
			variables[node.name][token.params[0].value] = variab
		"initglobal":
			if token.params.size() != 2 and token.params.size() != 3:
			...
			if token.params.size() == 3:
				variab.value = token.params[2].value
			if global_variables.has(token.params[0].value):
				push_warning('Line '+str(total_l)+': Variable '+token.params[0].value+' is already defined and will be overwritten')
		...
		"set":
			...
			if token.params[0].type != Lexer.TokenType.STRING:
				...
				return
			if !node.get_node_or_null(token.params[0].value) and token.params[0].value != "self":
			...
			if token.params[0].value != "self":
				target = node.get_node_or_null(token.params[0].value)
			if target.get_indexed(token.params[1].value) == null:
				push_error('Line '+str(total_l)+': Target node property must exist')
				return
			
			target.set_indexed(token.params[1].value,token.params[2].value)
		"tween_property":
			...
			if target.get_indexed(token.params[1].value) == null:
				push_error('Line '+str(total_l)+': Target node property must exist')
				return
			if token.params[3].type != Lexer.TokenType.NUMBER:
				push_error('Line '+str(total_l)+': Tween time must be a number')
				return
			
			var easing := Tween.EASE_IN_OUT
			if token.params.size() >= 5:
				if token.params[4].type != Lexer.TokenType.NUMBER:
					push_error('Line '+str(total_l)+': Tween ease must be a valid number or ease constant')
					return
				easing = [Tween.EASE_IN,Tween.EASE_OUT,Tween.EASE_IN_OUT,Tween.EASE_OUT_IN][token.params[4].value]
			#print(easing)
			var trans := Tween.TRANS_LINEAR
			if token.params.size() >= 6:
				if token.params[5].type != Lexer.TokenType.NUMBER:
					push_error('Line '+str(total_l)+': Tween transition must be a valid number or transition constant')
					return
				trans = [Tween.TRANS_LINEAR,Tween.TRANS_SINE,Tween.TRANS_QUINT,Tween.TRANS_QUART,Tween.TRANS_QUAD,Tween.TRANS_EXPO,Tween.TRANS_ELASTIC,Tween.TRANS_CUBIC,Tween.TRANS_CIRC,Tween.TRANS_BOUNCE,Tween.TRANS_BACK,Tween.TRANS_SPRING][token.params[5].value]
			if wait:
				await create_tween().tween_property(target,token.params[1].value,token.params[2].value,token.params[3].value).set_ease(easing).set_trans(trans).finished
			else:
				create_tween().tween_property(target,token.params[1].value,token.params[2].value,token.params[3].value).set_ease(easing).set_trans(trans)
		"get":
			if token.params.size() != 2:
			...
			return target.get_indexed(token.params[1].value)
		"sin":
			...
			if token.params[0].type != Lexer.TokenType.NUMBER:
				...
				return
			var output = sin(token.params[0].value)
			if token.params.size() == 2:
				if token.params[1].type != Lexer.TokenType.NUMBER:
					push_error('Line '+str(total_l)+': Output multiplier must be a number')
					return
				output *= token.params[1].value
		...
		"cos":
			...
			if token.params[0].type != Lexer.TokenType.NUMBER:
				push_error('Line '+str(total_l)+': Angle must be a number')
				return
			var output = cos(token.params[0].value)
			if token.params.size() == 2:
				if token.params[1].type != Lexer.TokenType.NUMBER:
					push_error('Line '+str(total_l)+': Output multiplier must be a number')
					return
		...
		"if":
			...
			token.params = await _convert_variables(token.params,[1])
			
			should_continue_block = true
			
			if token.params.size() == 1:
				# one parameter version
				should_continue_block = bool(token.params[0].value)
			else:
				# comparison
				if token.params[1].type != Lexer.TokenType.OPERATOR:
					push_error('Line '+str(total_l)+': Invalid comparison operator')
					return
				match token.params[1].value:
					"<=":
						should_continue_block = token.params[0].value <= token.params[2].value
					">=":
						should_continue_block = token.params[0].value >= token.params[2].value
					"!=":
						should_continue_block = token.params[0].value != token.params[2].value
		...
		"randi":
			...
			return randi_range(roundi(token.params[0].value),roundi(token.params[1].value))
		"randf":
			if token.params.size() != 2:
			...
			return randf_range(token.params[0].value,token.params[1].value)
		"round":
			if token.params.size() != 1:
			...
			return round(token.params[0].value)
